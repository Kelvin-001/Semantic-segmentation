...
class MultiResBlock(nn.Module):
    def __init__(self,U,alpha=1.67):
        ...
        self.W = alpha * U
        self.residual_layer = conv_block(14, int(self.W*0.167)+int(self.W*0.333)+int(self.W*0.5), 1, 1, 0, act=None)
        self.conv3x3 = conv_block(14, int(self.W*0.167))
...
if __name__ == "__main__":
    import torch
    model = MultiResUNet(14, 64, 10, True, True)
    input = torch.rand(1, 64, 512, 512)
    x = torch.rand(1, 128, 256, 256)
    st1 = model.conv_cat2(torch.cat((input, model.convd2_cat(model.upSample(x))), 1))
