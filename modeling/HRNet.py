...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, n_channels, n_filters, num_classes, config, **kwargs):
        ...
        # stem net
        self.conv1 = nn.Conv2d(n_channels, n_filters, kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.bn1 = BatchNorm2d(n_filters)
        self.conv2 = nn.Conv2d(n_filters, n_filters, kernel_size=3, stride=2, padding=1,
                               bias=False)
        self.bn2 = BatchNorm2d(n_filters)
        self.relu = nn.ReLU(inplace=True)
...
if __name__ == "__main__":
    ...
    model = HighResolutionNet(14, 64, 10, MODEL)
    # model.init_weights()
    ...
    st4 = nn.Conv2d(256, 256, kernel_size=3, stride=2, padding=1,bias=False)(st3)
    print(st4.detach().numpy().shape)
    st5 = nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True)(st4)
